...
const createStore = async (req, res) => {
  ...
  try {
    ...
    ).select("name contact status ownerId workingTime createdAt");
...
const updateStore = async (req, res) => {
  ...
  try {
    ...
    if (!store) return error(res, "No store found", 404);
...
const deleteStore = async (req, res) => {
  ...
  try {
    ...
    // Remove the storeId from the storeList of all admins
    await Admin.updateMany(
      { storeList: storeId }, // Match admins where storeList contains the storeId
      { $pull: { storeList: storeId } } // Pull the storeId from the storeList array
    );
